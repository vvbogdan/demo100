...
/**
 美颜
 */
@property(nonatomic, strong) AGBeautyFilter *beautyFilter;

/**
 礼物贴纸
 */
@property(nonatomic, strong) AGPresentStickerRenderer *presentStickerRenderer;

/**
 表情贴纸
 */
@property(nonatomic, strong) AGSmiliesStickerRenderer *smiliesStickerRenderer;

/**
 人脸点滤镜
 */
@property(nonatomic, strong) AGPointsRenderer *pointsRender;


@end

@implementation AGRenderManager

AGRenderManager *instanceManager = nil;
...
- (instancetype)initWithModelPath:(NSString *)modelPath isCameraPositionBack:(BOOL)cameraPositionBack {
    if (self = [super init]) {
        ...
        self.renderer = [[AGRenderer alloc] initWithModelPath:modelPath];
...
+ (int)renderInitCode {
    ...
    return [AGRenderer renderInitCode];
}
...
- (void)loadRender {
    ...
    //加载人脸贴纸
    [[AGStickerManager sharedManager] loadStickersWithCompletion:^(NSMutableArray<AGSticker *> *stickers) {
        __weakSelf.stickers = stickers;

        //刷新UI
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:@"AG_STICKERSLOADED_COMPLETE" object:nil];
        });
    }];


    //加载礼物贴纸
    [[AGPresentStickerManager sharedManager] loadStickersWithCompletion:^(NSMutableArray<AGSticker *> *stickers) {
    ...
    //加载全局滤镜
    [[AGGlobalFilterManager sharedManager] loadColorFiltersWithCompletion:^(NSMutableArray<AGGlobalFilter *> *filters) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"AG_COLORFILTERSLOADED_COMPLETE" object:nil];
        });
    ...
    self.smiliesStickerRenderer = [[AGSmiliesStickerRenderer alloc] init];

    self.presentStickerRenderer = [[AGPresentStickerRenderer alloc] init];

    //大眼瘦脸滤镜
    self.smallFaceBigEyeFilter = [[SmallFaceBigEyeFilter alloc] init];

    //美颜滤镜
    self.beautyFilter = [[AGBeautyFilter alloc] init];
...
//                               ];
    ...
    self.varWidth = ScreenWidth_AG;
    self.varHeight = ScreenHeight_AG;

    [self resetDistortionParams];

    //添加表情贴纸参数设置
    self.renderer.agRenderBlock =
            ^(unsigned char *pixels, int format, int width, int height, result_68_t *p_result, int rstNum, int orientation, int faceNum) {
                ...
                if (mouth_open && ![__weakSelf.renderer checkSmiliesSticker:__weakSelf.smiliesStickerRenderer] && __weakSelf.renderer.isEnableSmiliesSticker) {
                    ...
                    AGSticker *lastSticker = __weakSelf.presentStickers[0];
                    //设置播放次数
                    [lastSticker setPlayCount:1];
                    //贴纸帧数置零 将贴纸重新播放
                    for (AGStickerItem *item in lastSticker.items) {
...
+ (void)processPixelBuffer:(CVPixelBufferRef)pixelBuffer{
    ...
    switch (iDeviceOrientation) {
        ...
        case UIDeviceOrientationLandscapeLeft:
            ...
            [Global sharedManager].PIXCELBUFFER_ROTATE = AG_PIXELBUFFER_ROTATE_270;
            break;
        ...
        case UIDeviceOrientationPortraitUpsideDown:
            ...
            [Global sharedManager].PIXCELBUFFER_ROTATE = AG_PIXELBUFFER_ROTATE_180;
            break;
...
- (void)onDistortionChanged:(NSInteger)filterType {
    ...
    if (filterType >= 0) {
        self.currentDistortionFilter =
                (GPUImageOutput <GPUImageInput, AGRenderProtocol> *) [self.distortionFilters objectAtIndex:(filterType)];
...
- (void)onFilterChanged:(NSInteger)pos {
    ...
    if (pos >= 0) {
        ...
        AGGlobalFilter *filter = [self.globalFilters objectAtIndex:pos];
        NSLog(@"filter name:%@,type:%zd,dir:%@",filter.name,filter.type,filter.filterResourceDir);
...
- (void)onStickerChanged:(NSInteger)pos {
    ...
    if (self.currentStickerIndex >= 0) {
        ...
        AGSticker *lastSticker = self.stickers[self.currentStickerIndex];
...
- (void)onPresentStickerChanged:(NSInteger)pos {
    ...
    if (self.currentPresentStickerIndex >= 0) {
        AGSticker *lastSticker = self.presentStickers[self.currentPresentStickerIndex];
        //设置播放次数
        [lastSticker setPlayCount:1];
        //贴纸帧数置零 将贴纸重新播放
        for (AGStickerItem *item in lastSticker.items) {
 ...
 * value：参数调节值
 */
- (void)onNewBeautyParamsChanged:(AG_NEWBEAUTY_TYPE)type value:(float)value {
    if (type == AG_NEWBEAUTY_TYPE_EYEMAGNIFYING || type == AG_NEWBEAUTY_TYPE_CHINSLIMING) {
...
- (void)releaseManager {
    ...
    //release array
    [AGGlobalFilterManager removeAllColorFilters];
    self.currentDistortionFilter = nil;
    self.currentGlobalFilter = nil;
    self.stickers = nil;
